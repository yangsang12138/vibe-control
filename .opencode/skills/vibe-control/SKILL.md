...
此命令输出当前 Git 仓库的根目录绝对路径。**当前项目的所有文件均位于此路径下。**

### 0b. 边界规则

- ✅ **允许**：访问、读取、修改当前项目根目录下的任何文件
- 🚫 **禁止（未经确认）**：访问、读取、修改项目根目录之外的任何路径（包括但不限于 `../` 开头的相对路径、`/Users/.../OtherProject/` 等绝对路径）
- ⚠️ **如需越界**：必须先使用 `question` 工具让用户授权，获得明确许可后方可操作

### 0c. 自检命令

在不确定路径是否安全时，使用 scope-check.sh 自检：

```bash
bash vibe-control/scripts/scope-check.sh <目标路径>
# 返回 0 = 安全（项目内）
# 返回 1 = 需要用户授权
```
...
- **项目边界**：访问任何文件前确认路径在 `git rev-parse --show-toplevel` 内。跨项目访问必须先使用 `question` 工具获得用户授权。工具自检：`bash vibe-control/scripts/scope-check.sh <路径>`
