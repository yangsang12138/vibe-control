...
- **控制平面与业务代码分离**：控制文件通过 `git clone` 引入，`vibe-control/` 及所有注入产物均在 `.gitignore` 中，不提交到目标代码库
...
| 场景 | 命令 | 说明 |
|---|---|---|
| **首次接入** | 见下方代码块 | 克隆仓库 → 注入控制文件 → 安装钩子 → 配置 .gitignore |
| **升级控制脚本** | `sync-core.sh` | 拉取最新代码 → 重新注入，当 vibe-control 仓库有更新时使用 |
...
```bash
git clone https://github.com/yangsang12138/vibe-control.git
...
| `scripts/init.sh` | **一键初始化**，克隆仓库 + 注入 + 软链 |
...
| `scripts/sync-core.sh` | **同步核心文件**，拉取最新代码 + 刷新软链 |
...
| `scripts/scope-check.sh` | 项目边界检测，判断指定路径是否在当前仓库内 |
| `scripts/scan-modules.sh` | 项目模块依赖扫描，根据源码 import 自动生成模块关系图 |
